  ...
  <div class="max-w-full overflow-x-auto custom-scrollbar">
    ...
    <div>
      <div id="chartOne" class="-ml-5">
        <VueApexCharts type="line" height="350" :options="apexOptions" :series="chartData.series" ref="chart" />
      </div>
    </div>
  </div>
  </div>
...
async function fetchChartData(period, backendUrl) {
  try {
    const response = await axios.get(
      `${backendUrl}/api/measurements?interval=${period}`
    )
    ...
    labels.forEach(date => {
      Object.values(apiData[date]).forEach(m =>
        measurementTypesSet.add(m.measurement_type_string)
      )
    })
    ...
    seriesData = measurementTypes.value.map(type => ({
      ...
      data: labels.map(date => {
        ...
        const measurement = dayData.find(
          m => m.measurement_type_string === type
        )
